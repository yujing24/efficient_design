...
## Assessment functions for sensitivity analysis 3
{
  
  # Function for assessment in sufficient EC case (difference-in-means, RCT-only AIPW and hybrid design)
  f_assessment_sensi3 <- function(n, # RCT sample size
                                  N_EC = 1000, # EC data sample size 
                                  M = 2000, # simulation times
                                  alpha = 0.05, # type I error for two-sided hypothesis test
                                  tau = 0, # true ATT, 0 for type I error and 0.4 for power 
                                  delta_U = 0, # 0, 0.5 or 1
                                  betaU = 0.3,
                                  seed = 123, # random seed for generating trial data
                                  pi_A = 0.5 # treatment allocation in RCT data
  ){
    ...
    for(i in 1:M){
      ...
      data_EC <- f_gen_EC_confound(seed_EC = Index_all[i],
                                   ...
                                   betaU = betaU,
                                   delta_U = delta_U)
      data_RCT <- f_gen_RCT_confound(seed_RCT = Index_all[i],
                                     N_RCT = n,
                                     pi_A = pi_A,
                                     tau = tau,
                                     betaU = betaU)
      ...
      res_EC <- try(f_est_hybrid(data = data_full), silent = TRUE)
      if(inherits(res_EC, "try-error") || length(res_EC) == 1){
        res_EC <- data.frame(Est_EC = NA,  
                             Est_EC_var = NA)
      }
      res_AIPW <- try(f_est_AIPW(data = data_RCT), silent = TRUE)
      if(inherits(res_AIPW, "try-error") || length(res_AIPW) == 1){
        res_AIPW <- data.frame(Est_AIPW = NA, 
                               Est_AIPW_var = NA)
      }
      res_std <- f_est_naive(data = data_RCT)
    ...
    res <- data.frame(power_EC = sum(res_IF_EC, na.rm = T) / M,
                      power_AIPW = sum(res_IF_AIPW, na.rm = T) / M,
                      power_naive =  sum(res_naive, na.rm = T) / M)
  ...
  # Function for assessment in sufficient EC case (single-arm design)
  f_assessment_sa_sensi3 <- function(n, # RCT sample size
                                     N_EC = 1000, # EC data sample size 
                                     M = 2000, # simulation times
                                     alpha = 0.05, # type I error for two-sided hypothesis test
                                     tau = 0, # true ATT, 0 for type I error and 0.4 for power
    ...
    for(i in 1:M){
      ...
      res_SA <- try(f_est_sa(data_full), silent = TRUE)
      if(inherits(res_SA, "try-error") || length(res_SA) == 1){
        res_SA <- data.frame(Est_SA = NA,  
                             Est_SA_var = NA)
      }
...
## Assessment functions for sensitivity analysis 4
{
  
  # Function for assessment in insufficient EC case (difference-in-means, RCT-only AIPW and hybrid design)
  f_assessment_sensi4 <- function(n, # RCT sample size
                                  N_EC = 1000, # EC data sample size 
    ...
    res_naive <- (Result$p_value < alpha) *1
  ...
  # Function for assessment in sufficient EC case (single-arm design)
  f_assessment_sensi4_sa <- function(n, # RCT sample size
                              N_EC = 1000, # EC data sample size 
                              M = 2000, # simulation times
                              alpha = 0.05, # type I error for two-sided hypothesis test
                              tau = 0, # true ATT, 0 for type I error and 0.4 for power 
...
## Assessment functions for sensitivity analysis 5
{
  
  f_assessment_sensi5 <- function(n, # RCT sample size
                                  N_EC = 1000, # EC data sample size 
                                  M = 1000, # simulation times
                                  alpha = 0.05, # type I error for two-sided hypothesis test
                                  tau = 0, # true ATT, 0 for type I error and 0.4 for power 
    ...
    Result <- c()
    for(i in 1:M){
      data_EC <- f_gen_EC_sensi5(seed_EC = Index_all[i],
                                 N_EC = N_EC,
                                 muX1_EC = 1, 
                                 sdX1_EC = 1,
                                 pX2_EC = 0.5,
                                 ...
                                 beta2_EC = -1,
                                 sdY_EC = 1)
      data_RCT <- f_gen_RCT_sensi5(seed_RCT = Index_all[i],
                                   N_RCT = n,
                                   pi_A = pi_A,
                                   tau = tau,
                                   muX1_R = 1, 
  ...
  f_assessment_sensi5_sa <- function(n, # RCT sample size
                                     N_EC = 1000, # EC data sample size 
                                     M = 1000, # simulation times
                                     alpha = 0.05, # type I error for two-sided hypothesis test
                                     tau = 0, # true ATT, 0 for type I error and 0.4 for power 
