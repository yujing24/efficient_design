...
####### Simulation to verify the smallest sample size of N_RCT #######
{
  ...
  # Step 1: 
  {
    ...
    f_gen_EC_Y <- function(x,
                           ...
                           sdY){
      mu <- beta0 + beta1 * x 
      return(mu + rnorm(1, mean = 0, sd = sdY))
    }
    
    # Generate EC data
    f_gen_EC <- function(seed_EC,
                         N_EC = 1000,
                         muX_EC = 1, 
                         sdX_EC = 1,
                         beta0 = 1,
  ...
  # Step 2
  {
    ...
    gamma = 1 # the correlation 
      ...
      term1 <- N / N_RCT / pi_A * kappa_1
      term2 <- N / N_RCT * ((1 - pi_A) * var_X_RCT_0) / ((1 - pi_A) + r / d_X / r_R)^2 
      ...
      term6 <- N / N_RCT * r^2  * var_X_EC / r_R / ((1 - pi_A) + r / d_X / r_R)^2
    ...
    f_est_full <- function(data = data_full){
      ...
      return(data.frame(IF_est_tau_full = IF_est_tau,  
                        IF_est_var_full = IF_est_var))
    }
    ...
    f_est_RCT <- function(data = data_RCT){
      ...
      return(data.frame(IF_est_tau_RCT = IF_est_tau,  
                        IF_est_var_RCT = IF_est_var))
    }
  ...
  # Step 3:
  {
    ...
    # pi = 0.5 & N_RCT = 124
    {
      ...
      for(i in 1:M){
        ...
        res1 <- f_est_full(data_full)
        res2 <- f_est_RCT(data_RCT)
        res3 <- f_est_naive(data_RCT)
        
        res4 <-  data.frame(IF_est_tau_simple = res1$IF_est_tau_full, 
                            est_var_simple = 
                              Est_var_simple(data_EC, 
                                             N_RCT = n,
                                             pi_A = pi_A)[1])
    ...
    # Hypothesis H0: tau = 0
    {
      ...
      Z_IF_full <- (Result$IF_est_tau_full - 0) / sqrt(Result$IF_est_var_full / N)
      res_IF_full <- (abs(Z_IF_full) > qnorm(1 - alpha/2)) * 1
      
      Z_IF_RCT <- (Result$IF_est_tau_RCT - 0) / sqrt(Result$IF_est_var_RCT / n)
      res_IF_RCT <- (abs(Z_IF_RCT) > qnorm(1 - alpha/2)) * 1
      
      res_naive <- (Result$p_value < 0.05) *1
      
      Z_IF_simple <- (Result$IF_est_tau_simple - 0) / sqrt(Result$est_var_simple / N)
      res_IF_simple <- (abs(Z_IF_simple) > qnorm(1 - alpha/2)) * 1
      
      res <- data.frame(power_IF_full = sum(res_IF_full) / M,
                        power_IF_RCT = sum(res_IF_RCT) / M,
                        power_naive =  sum(res_naive) / M,
                        power_IF_simple = sum(res_IF_simple) / M)
    }
